...
These variables are declared directly inside the `.github/workflows/deploy.yml` file as text strings. **CRITICAL:** Before triggering your first pipeline run, you must open your workflow file and update these strings to match the resource outputs generated by your Terraform deployment:

* **`ACR_NAME`:** The unique name of your Azure Container Registry (e.g., change `enclaveacr1h4eja` to your unique registry string)
